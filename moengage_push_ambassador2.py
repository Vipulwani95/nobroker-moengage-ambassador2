...
# CONFIGURATION
# ==============================
# Local runs: falls back to the JSON file on disk.
# GitHub Actions runs: set GOOGLE_SERVICE_ACCOUNT_JSON secret to the
# full contents of the service account JSON (not base64, just raw JSON).
credentials_path = 'D:/Downloads/db-mismath-starship-data-ff9d8efaf2bb.json'
GOOGLE_SERVICE_ACCOUNT_JSON = os.environ.get("GOOGLE_SERVICE_ACCOUNT_JSON")

SOURCE_SHEET_NAME = "Acquisition Campaigns and Calling Ambassador-2"
SOURCE_TAB_NAME = "FB Campaign_Acquisition_Apr26"
...
# Pause after every 1000 uploads (only relevant for interactive/local runs)
UPLOAD_CHUNK_SIZE = 1000

# When true (CI / GitHub Actions), never prompt for input — just run
# straight through all chunks.
...
# uploads to the sheet (still logs what WOULD happen). Handy for
# verifying the filter + workflow before a real run.
...
COL_DATE = 14      # O -> must be today, format like "1-Apr-2026"
...
if GOOGLE_SERVICE_ACCOUNT_JSON:
    ...
    # as an env var) — build credentials from the JSON string directly.
    ...
    # Local run — fall back to the file on disk.
...
source_sheet = client.open(
    SOURCE_SHEET_NAME
).worksheet(SOURCE_TAB_NAME)

# Raw values, NOT get_all_records(), since we're addressing by column
# letter/index rather than by header name.
all_source_values = source_sheet.get_all_values()

print(f"📊 Total rows loaded from source: {len(all_source_values) - HEADER_ROWS}")
...
try:
    ...
    dest_sheet = client.open(
        SOURCE_SHEET_NAME
    ).worksheet(DEST_TAB_NAME)
...
if len(all_dest_values) > 1:
    ...
    headers_row = [
        str(h).strip()
        for h in all_dest_values[0]
    ]

    try:

        phone_col_index = headers_row.index(
            DEST_PHONE_COL
        )
        ...
        print(
            f"❌ '{DEST_PHONE_COL}' column not found in dest tab"
        )

print(
    f"📋 Already uploaded numbers found: "
    f"{len(uploaded_phones)}"
)
...
def is_date_in_range(date_val_str):
    ...
    start_date = ist_today - timedelta(days=7)

    return start_date <= parsed <= ist_today


def row_matches_filter(row):
...
def build_payload(row):
    ...
    attributes = {
        ...
        "upload_time_readable": now.strftime(
            "%Y-%m-%d %H:%M:%S"
        )
    }
...
for row in all_source_values[HEADER_ROWS:]:
...
print(
    f"⚠️ Invalid phone rows      : "
    f"{len(invalid_rows)}"
)

print(
    f"⏭️ Skipped (filter no match): "
    f"{skipped_rows}"
)
...
for chunk_start in range(
    ...
    print(
        ...
        f"{chunk_start + 1} "
        f"to "
        f"{min(chunk_end, len(eligible_rows))}"
    )
